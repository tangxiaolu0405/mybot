...
func drainLines(label string, r io.Reader) {
	sc := bufio.NewScanner(r)
	for sc.Scan() {
		// MCP servers may log to stderr; keep quiet unless debugging
		_ = label
	}
}
...
func (c *stdioClient) callTool(ctx context.Context, toolName string, args map[string]interface{}) (string, error) {
	...
	if res.IsError {
		return formatToolContent(res.Content), fmt.Errorf("mcp tool error")
	}
